...
public class Example5_Zones : AbstractExample
{
    CKDatabase database;
    ...
    private void OnRecordZoneCreated(CKRecordZone zone, NSError error)
    {
        EnqueueOnMainThread(() =>
        {
            ...
            var save1 = CKRecord.initWithRecordType("SaveGame", customZone.ZoneID);

            // or if you need a custom record name, initialize with a recordID with the zoneId
            var save2 = CKRecord.initWithRecordType("SaveGame", CKRecordID.initWithRecordName("MySave", customZone.ZoneID));

            var op = CKModifyRecordsOperation.initWithRecordsToSave(new CKRecord[] { save1, save2 }, null);
    ...
    private void DeleteZone()
    {
        ...
        var op = CKModifyRecordZonesOperation.initWithRecordZonesToSave(null, new CKRecordZoneID[] { customZone.ZoneID });
        op.Configuration.QualityOfService = NSQualityOfService.UserInitiated;
